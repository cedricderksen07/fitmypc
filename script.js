...
// Optionale Debug-/Konsolen-Helfer
try {
    window.init = function(){
        try { initializeConfigurator(); } catch(e) { console.warn('initializeConfigurator failed', e); }
        try { updateAccountLink(); } catch(e) { console.warn('updateAccountLink failed', e); }
        try { initializePreBuiltPCs(); } catch(e) { console.warn('initializePreBuiltPCs failed', e); }
        try { initializeAccessories(); } catch(e) { console.warn('initializeAccessories failed', e); }
        return 'Init complete';
    };
    window.debugRemoveSiteBlocker = function(){
        const el = document.getElementById('siteBlocker');
        if (el) el.remove();
        document.documentElement.style.overflow = '';
        document.body.style.overflow = '';
        return 'Site blocker removed';
    };
    Object.defineProperty(window, 'ccOpen', {
        get(){ return (window.CookiePrefs && typeof window.CookiePrefs.open === 'function') ? window.CookiePrefs.open : undefined; }
    });
} catch(e) {}
